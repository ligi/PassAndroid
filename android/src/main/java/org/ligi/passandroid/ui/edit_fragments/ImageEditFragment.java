...
public class ImageEditFragment extends PassandroidFragment {
    ...
    private void extractImage(Intent imageReturnedIntent, String name) {
        final File extractedFile = AXT.at(imageReturnedIntent.getData()).loadImage(getContext());
        if (extractedFile != null) {
            try {
                Files.copy(extractedFile, new File(passStore.getPathForID(getPass().getId()), name + PassImpl.Companion.getFILETYPE_IMAGES()));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
